...
// call a local model that also runs in the browser (e.g. a transformers.js
// text-generation model via WebGPU), feeding it `query` + the `passages` as grounding
// context and asking it to cite [n]. Nothing else in the app has to change — the
// contract (an answer string + the citation list) stays identical.
...
// Placeholder for Phase 2. Wire a browser-local model here and flip MODE to 'local-llm'.
