...
import { embed, fromBlob } from './embedder.js';
...
function buildIndex(notebookId) {
  const rows = getNotebookChunks(notebookId);
  const chunks = rows.map((r) => ({
    ...
    locator: r.locator,
    text: r.text,
    vector: r.embedding ? fromBlob(r.embedding) : null,
  }));
...
const getIndex = (notebookId) => cache.get(notebookId) || buildIndex(notebookId);
...
export async function search(notebookId, query, k = 6) {
  const index = getIndex(notebookId);
  if (index.chunks.length === 0) return { results: [], totalChunks: 0 };
